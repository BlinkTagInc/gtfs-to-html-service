...
This project is a next.js app that runs on a server and uses [GTFS-to-HTML](https://github.com/BlinkTagInc/gtfs-to-html) to generate HTML or CSV schedules from transit data in GTFS format.
...
Each ticket can authorize one upload token and one generation attempt. Private
Blob markers use conditional creation (`allowOverwrite: false`) to arbitrate
claims across instances without Redis. Claims remain consumed on errors; retry
with a new upload. Cleanup replaces consumed uploads with empty placeholders
so unexpired no-overwrite client tokens cannot recreate their paths. The daily
cron removes placeholders and claim markers after 24 hours. This adds two small
Blob writes per upload plus a cleanup overwrite. Provider outages fail claims
closed; cleanup failures are logged and retried by cron.
